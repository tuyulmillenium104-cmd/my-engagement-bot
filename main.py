...
async def process_payment(data, approved):
    ...
    req_data = await load_json('requests.json', dict)
    if request_id not in req_data:  # ✅ PERBAIKAN: ganti req_ → req_data
        return
    ...
    if not approved:
        if is_comment and task_idx is not None:
            ...
            # Tidak hapus like/retweet/follow — sekali klaim, tetap tercatat
            pass
...
async def on_reaction_add(reaction, user):
    ...
    req_data = await load_json('requests.json', dict)
    msg_id = str(reaction.message.id)
    if msg_id not in req_data:  # ✅ PERBAIKAN: ganti req_ → req_data
        return
    ...
    if task_type in ("like", "retweet"):
        ...
        # Follow: sekali seumur hidup per pasangan user (bukan per link)
...
async def buy_engagement(ctx, days: int, link: str, *, comments_raw: str = ""):
    ...
    if current_points < total_price:
        ...
        await ctx.message.delete()  # ✅ PASTIKAN INI ADA!
        return
